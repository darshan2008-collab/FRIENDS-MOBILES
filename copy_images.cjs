...
const mappings = [
  { srcDir: currentBrainDir, srcFile: 'hero_showcase_ai_1784640427247.png', destName: 'hero_devices.png' },
  { srcDir: currentBrainDir, srcFile: 'hero_showcase_ai_1784640427247.png', destName: 'hero_devices_light.png' },
  { srcDir: currentBrainDir, srcFile: 'hero_showcase_dark_ai_1784640456103.png', destName: 'hero_devices_dark.png' },
...
function copyAllImages() {
  try {
    ...
    mappings.forEach(item => {
      ...
      const fallbackSrc = path.join(__dirname, 'src/assets', item.destName);

      if (fs.existsSync(srcPath)) {
        targetDirs.forEach(dir => {
          try { fs.copyFileSync(srcPath, path.join(dir, item.destName)); } catch (e) {}
        });
      } else if (fs.existsSync(fallbackSrc)) {
        targetDirs.forEach(dir => {
          try { fs.copyFileSync(fallbackSrc, path.join(dir, item.destName)); } catch (e) {}
        });
      }
    });
  } catch (err) {
